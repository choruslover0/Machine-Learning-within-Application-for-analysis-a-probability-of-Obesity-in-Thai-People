...
**Date:** 2026-05-30  
**Branch:** main  
**Commits:** 17 commits on top of `a874392`
...
- **Style:** Rounded-square FAB (68 × 68 px) in the O-Beast gradient, bottom-right corner, `position: fixed`
...
- **EN / TH toggle** in the header — switches chip labels, placeholder text, and bot answers
- **Quick-reply chips** (Causes / Prevention / Diet / Exercise) that shrink to a compact row after the first message
...
After submitting the predictor form, the widget FAB receives `data-risk-tier` and `data-probability` attributes. The chat header shows a pink banner: *"📊 Your result: Very High Risk (85%) — I'll tailor my answers to your score."* Every `/chat` request from that page includes the context so answers are personalised.

### 7. Notification System (Result + Advice pages)

When a user receives a prediction result or wellness advice:

- After **1.5 seconds**, the Beast 1.0 FAB **shakes** (wiggle animation)
- A speech bubble appears above the FAB: **"If you want any extra answers I'm here 👋"**
- The bubble has its own **×** dismiss button and auto-hides after 6 seconds
...
| Chat window too tall — extended above top of screen | Added `max-height: calc(100vh - 140px)` + `display: flex; flex-direction: column` |
| Chips stayed large after selection | Added `.compact` CSS class applied via JS on first message send |
...
| File | What Changed |
|---|---|
| `src/obesity_ml/chatbot.py` | New file — knowledge base, classifier, language detection, `chat()`, `train_chatbot()` |
| `src/obesity_ml/app.py` | `POST /chat` endpoint, `page_shell()` updated, full chat widget HTML/CSS/JS |
...
| `tests/test_chatbot.py` | New — 40 tests covering config, data, knowledge base, classifier, endpoint, widget |
...
| `ChatbotWidgetTests` | FAB + chat window appear on all 4 pages |
...
1. Click the **Beast 1.0** button (bottom-right) to open the chat
2. Tap a chip or type a question in English or Thai
3. Submit a prediction — after 1.5 s the FAB shakes and invites you to ask follow-up questions
